...
pub struct QueuedTask {
    ...
    // Market extensions
    pub bid_amount: u64,        // Tokens offered this auction tick
    pub treasury_balance: u64,  // Token balance snapshot before bid
    pub starvation_ns: u64,     // Nanoseconds since last CPU dispatch
    pub class_hint: u32,        // process_class_t
    pub numa_node: u32,         // NUMA topology domain
}
...
pub struct DispatchedTask {
    ...
    pub enq_cnt: u64,
    // Market extensions
    pub clearing_price: u64, // Tokens deducted from winner's treasury
    pub sequence: u64,       // Monotonic auction cycle counter
}
...
impl<'cb> BpfScheduler<'cb> {
    ...
    pub fn init(
        ...
        let mut skel = scx_ops_open!(skel_builder, open_object, agentic, open_opts)?;
        ...
        skel.struct_ops.agentic_mut().flags =
            *compat::SCX_OPS_ENQ_LAST | *compat::SCX_OPS_ALLOW_QUEUED_WAKEUP;
        if partial {
            skel.struct_ops.agentic_mut().flags |= *compat::SCX_OPS_SWITCH_PARTIAL;
        }
        if numa_local {
            skel.struct_ops.agentic_mut().flags |= *compat::SCX_OPS_BUILTIN_IDLE_PER_NODE;
        }
        skel.struct_ops.agentic_mut().exit_dump_len = exit_dump_len;
        ...
        let _ = Self::set_scx_ops_name(&mut skel.struct_ops.agentic_mut().name, name);

        // Attach BPF scheduler.
        let mut skel = scx_ops_load!(skel, agentic, uei)?;

        let struct_ops = Some(scx_ops_attach!(skel, agentic)?);
