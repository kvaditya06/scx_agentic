...
/*
 * Transfer requests: parent -> child token donations.
 * Rust Clearinghouse polls and processes these.
 */
struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__type(key, s32);            /* child pid */
 ...
 * Tracks number of transfers in the current window.
 */
struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__type(key, s32);            /* parent pid */
	__type(value, u64);          /* transfers in current window */
	__uint(max_entries, MAX_TRACKED_PIDS);
 ...
 * Shadow cores run EEVDF as a control group for telemetry.
 */
struct {
	__uint(type, BPF_MAP_TYPE_ARRAY);
	__type(key, u32);
...
struct task_ctx {
	...
	/*
	 * Timestamp of last successful dispatch (for starvation tracking).
	 */
	u64 last_dispatch_ts;
};
...
s32 BPF_STRUCT_OPS(agentic_select_cpu, struct task_struct *p, s32 prev_cpu,
		   u64 wake_flags)
{
	s32 cpu, this_cpu = bpf_get_smp_processor_id();
	bool is_this_cpu_allowed = bpf_cpumask_test_cpu(this_cpu, p->cpus_ptr);
			  ...
			  struct task_ctx *tctx, u64 enq_flags, s32 prev_cpu)
{
	...
	u64 now = scx_bpf_now();
	s32 pid_key = p->pid;
	struct treasury_entry *te = bpf_map_lookup_elem(&treasury, &pid_key);
 ...
 * scheduler.
 */
void BPF_STRUCT_OPS(agentic_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	/*
	 * Circuit breaker check: if the market is tripped, bypass the
	 * userspace scheduler and dispatch directly using vtime ordering.
	 */
	{
		s32 task_numa = get_task_numa_node(p);
		u32 numa_key = (task_numa >= 0) ? (u32)task_numa : 0;
	...
	/*
	 * Shadow core routing: 5% random sample bypasses market and
	 * dispatches via EEVDF for telemetry comparison.
	 */
	{
		u32 shadow_cpu_key = prev_cpu;
		u8 *is_shadow = bpf_map_lookup_elem(&shadow_cores, &shadow_cpu_key);
		if (is_shadow && *is_shadow) {
			/* Already on a shadow core — dispatch via EEVDF */
			scx_bpf_dsq_insert_vtime(p, SHARED_DSQ,
						 slice_ns, p->scx.dsq_vtime, enq_flags);
			__sync_fetch_and_add(&nr_kernel_dispatches, 1);
			goto out_kick;
		...
		if (bpf_get_prandom_u32() % 20 == 0) {
			/* 5% sample: route to shadow core via EEVDF */
			scx_bpf_dsq_insert_vtime(p, SHARED_DSQ,
						 slice_ns, p->scx.dsq_vtime, enq_flags);
			__sync_fetch_and_add(&nr_kernel_dispatches, 1);
			goto out_kick;
 ...
 */
void BPF_STRUCT_OPS(agentic_dispatch, s32 cpu, struct task_struct *prev)
{
	/*
...
void BPF_STRUCT_OPS(agentic_runnable, struct task_struct *p, u64 enq_flags)
{
	struct task_ctx *tctx;
 ...
 */
void BPF_STRUCT_OPS(agentic_running, struct task_struct *p)
{
 ...
 */
void BPF_STRUCT_OPS(agentic_stopping, struct task_struct *p, bool runnable)
{
	u64 now = scx_bpf_now();
	s32 cpu = scx_bpf_task_cpu(p);
	struct task_ctx *tctx;
	...
	/* Increment per-CPU context-switch counter for circuit breaker */
	u32 csw_key = 0;
	u64 *csw_count = bpf_map_lookup_elem(&csw_counter, &csw_key);
	if (csw_count)
 ...
 */
void BPF_STRUCT_OPS(agentic_enable, struct task_struct *p)
{
	p->scx.dsq_vtime = 0;
 ...
 */
s32 BPF_STRUCT_OPS_SLEEPABLE(agentic_init)
{
	int err;
 ...
 */
void BPF_STRUCT_OPS(agentic_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}
 ...
 */
SCX_OPS_DEFINE(agentic,
	       .select_cpu		= (void *)agentic_select_cpu,
	       .enqueue			= (void *)agentic_enqueue,
	       .dispatch		= (void *)agentic_dispatch,
	       .runnable		= (void *)agentic_runnable,
	       .running			= (void *)agentic_running,
	       .stopping		= (void *)agentic_stopping,
	       .enable			= (void *)agentic_enable,
	       .init_task		= (void *)agentic_init_task,
	       .init			= (void *)agentic_init,
	       .exit			= (void *)agentic_exit,
	       .timeout_ms		= 5000,
